...
if json and json.encode then
player:sendExtendedOpcode(AutoLoot.OPCODE, json.encode({ action = "error", message = msg }))
end
end
...
if it and it:getId() > 0 then itemId = num end
else
local it = ItemType(tostring(input):lower())
if it and it:getId() > 0 then itemId = it:getId() end
end
...
local itemId = 0
local num = tonumber(input)
if num and num > 0 then itemId = num
else
local it = ItemType(tostring(input):lower())
if it and it:getId() > 0 then itemId = it:getId() end
end

if itemId == 0 then return false, "Item '" .. tostring(input) .. "' nao encontrado." end
