-- Sistema de Market via Comandos de Chat (yurOTS 7.72)
...
param = param:sub(2):gsub("^%s+", "")
end

-- Menu de Ajuda (!shop) -> Aparece direto na aba Default
if words == "!shop" or words == "/shop" then
local text = "=== [ MERCADO GLOBAL - AJUDA ] ===\n" ..
"1. Anunciar item: Coloque o item no Arrow Slot e digite: !shopsell <preco>\n" ..
"2. Ver ofertas: !shopoffers (ou !shopoffer)\n" ..
"3. Comprar item: !shopbuy <id>\n" ..
"4. Cancelar anuncio: !shopremove <id>\n" ..
"=================================="
...
-- 1. !shopsell <preco>
if words == "!shopsell" or words == "/shopsell" then
local price = tonumber(param)
if not price or price <= 0 then
player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Uso incorreto. Use: !shopsell <preco> (Exemplo: !shopsell 5000). Coloque o item no Arrow Slot!")
return false
end

local item = player:getSlotItem(10) -- 10 = CONST_SLOT_AMMO
if not item then
player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Erro: Voce nao tem nenhum item no slot de flecha (arrow slot) para vender.")
return false
end
...
player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Oferta criada com sucesso! " .. amount .. "x " .. itemName .. " anunciado por " .. price .. " gold coins.")
return true

-- 2. !shopoffers / !shopoffer
elseif words == "!shopoffers" or words == "!shopoffer" or words == "/shopoffers" or words == "/shopoffer" then
...
local text = "=== MERCADO GLOBAL - OFERTAS ATIVAS ===\n\n"
repeat
...
text = text .. "ID [" .. id .. "] | " .. amount .. "x " .. itemName .. "\nVendedor: " .. seller .. "\nPreco: " .. price .. " gps\nComprar: !shopbuy " .. id .. "\n-----------------------------------\n"
...
elseif words == "!shopremove" or words == "!shopcancel" or words == "/shopremove" or words == "/shopcancel" then
local offerId = tonumber(param)
if not offerId then
player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Uso incorreto. Use: !shopremove <id_da_oferta> (Exemplo: !shopremove 1).")
return false
end
...
-- 4. !shopbuy <id>
elseif words == "!shopbuy" or words == "/shopbuy" then
local offerId = tonumber(param)
if not offerId then
player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Uso incorreto. Use: !shopbuy <id_da_oferta> (Exemplo: !shopbuy 1).")
return false
end
