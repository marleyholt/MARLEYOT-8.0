...
function newDailyMonster()
	...
	if resultId ~= false then
		...
		db.asyncQuery("REPLACE INTO `server_config` (`config`, `value`) VALUES ('boosted_monster', '".. monsterName .."'), ('boosted_bonus', '".. monsterBonus .."')")
		daily = {name=monsterName,bonus=monsterBonus}
		result.free(resultId)
	else
		local resFallback = db.storeQuery("SELECT `id`,`name` FROM `monsters` ORDER BY RAND() LIMIT 1")
		if resFallback ~= false then
			local monsterId = result.getDataInt(resFallback, "id")
			local monsterName = result.getDataString(resFallback, "name")
			local monsterBonus = 100
